...
//Helper functions
Node newNode(void* data) { //Node constructor function.
	Node N;
...
bool isEmpty(List L) { //Returns true if empty.
	if(L == NULL) { //If List L is empty
		fprintf(stderr, "isEmpty List is empty\n");
		exit(EXIT_FAILURE);
	}
...
void* front(List L) { //Returns the front element of List L.
...
void* back(List L) { //Returns the back element of List L.
	if(L == NULL) { //Check if List L is NULL.
        fprintf(stderr, "Back List is null.\n");
        exit(EXIT_FAILURE);
    }
    if(isEmpty(L)) { //Check if List L is empty.
        fprintf(stderr, "Back List is empty.\n");
        exit(EXIT_FAILURE);
	}
	if(length(L) < 0) { //Pre: length() > 0
        fprintf(stderr, "Length precondition is not met.\n");
...
void* get(List L) { //Returns the cursor element of List L.
	if(L == NULL) { //Check if List L is NULL.
        fprintf(stderr, "Get List is null.\n");
        exit(EXIT_FAILURE);
    }
    if(isEmpty(L)) { //Check if List L is empty.
        fprintf(stderr, "Get List is empty.\n");
        exit(EXIT_FAILURE);
    }
...
void set(List L, void* x) { //Overwrites cursor element.
	if(L == NULL) { //Check if List L is NULL.
...
void movePrev(List L) { //Move cursor one step foward.
	...
	if(isEmpty(L)) { //Check if List L is empty.
		fprintf(stderr, "Moveprev List is empty.\n");
		exit(EXIT_FAILURE);
	}
...
void moveNext(List L) { //Move cursor one step backward.
	...
	if(isEmpty(L)) { //Check if List L is empty.
		fprintf(stderr, "Move next List is empty.\n");
		exit(EXIT_FAILURE);
	}
...
void prepend(List L, void* x) { //Insert new element into List L before front element.
	Node N = newNode(x); //Create Node N.
...
void append(List L, void* x) { //Insert new element into List L after front element.
	Node N = newNode(x); //Create Node N.
	N->data = x; //Set Node N data to x.
...
void printList(FILE* out, List L) { //Prints to outfile.
	...
	for(moveFront(L); index(L) >= 0; moveNext(L)) {
		fprintf(out, "%p ", get(L)); //Print Node N data into  outfile.
	}
}
